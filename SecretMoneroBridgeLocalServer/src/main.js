...
const debug = false;
const dappLocation = "k2k4r8l576wm2szxbondf3zs9nqlroz32jccdkbb1j4p130uzz7jnacr";
    ...
    if (gateways && gateways.length > 0) {
      ...
      for (let x = 0; x < gateways.length; x++) {
        try {
          ...
          data.location = `${gateways[x].value.gate.replace(replaceThis, replaceThat)}`;
          resolve(data)
          break;
        } catch (ignore) { }
...
const queryIpfs = async () => {
  return new Promise(async (resolve, reject) => {
    ...
    const getFile = (url, savepath,protocol)=>{
      return new Promise(async (resolve, reject) => {
        const request = protocol.get(url, function (response) {
          if (response.statusCode === 200) {
            var file = fsE.createWriteStream(savepath);
            response.pipe(file);
            resolve()
          }
    ...
    const getFileList = (url, list) => {
      return new Promise(async (resolve, reject) => {
        const filepath = path.join(homepath, "public");
        fsE.ensureDirSync(filepath);
        fsE.ensureDirSync(filepath + '/static');
        fsE.ensureDirSync(filepath + '/static/js');
        fsE.ensureDirSync(filepath + '/static/css');
        fsE.ensureDirSync(filepath + '/static/media');

        let filePromises = []

        for (let x = 0; x < list.length; x++) {
          filePromises.push(getFile(url +'/' + list[x], path.join(homepath, "public", list[x]),url.indexOf('https')!==-1?https:http));
        };
    ...
    const getSize = async () => {
      return new Promise(async (resolve, reject) => {
        try {
          const filelistData = await getDappFilelist("ipfs/:hash", `ipns/${dappLocation}`,"filelist.json", "ipns");
          
          resolve(filelistData)
        } catch (error) {
          reject();
        }
      });
    ...
    try {
      const publicFolderExists = await fsE.pathExists(path.join(homepath, "public"));
      const doUpdate = async (filesLocation, remoteFiles) => {
        await removedir(path.join(homepath, "public"))
        await removedir(path.join(homepath, "latesthash"))
        // await processIPFSFiles(hash)
        await getFileList(filesLocation, remoteFiles);
      };

      mainWindow.webContents.send("updatestatus", { text: "Checking IPFS Gateways, please be patient." })
      // const ipfsGood = await initIpfs();

      // if (ipfsGood) {

        const publicFilelist = publicFolderExists?await fsE.readJson(path.join(homepath, "public", "filelist.json")):null;

        // contentcid = await getContentVars();

        const remoteFileList = await getSize();
        const remoteFiles = remoteFileList.files
        remoteFiles.push("filelist.json");
        const filelistSize = remoteFileList.size;
        const filesLocation = remoteFileList.location;
        ...
        // if (contentcid) {
          if (!publicFolderExists) {
            ...
            await doUpdate(filesLocation, remoteFiles)
